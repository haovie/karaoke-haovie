...
export const QueuePanel: React.FC = () => {
  ...
  if (queue.length <= currentIndex + 1) return null;

  const upNext = queue.slice(currentIndex + 1, currentIndex + 6);
  const remaining = Math.max(0, queue.length - (currentIndex + 6));

  return (
    <div className="absolute top-12 right-12 w-96 bg-black/80 backdrop-blur-md border border-gray-700 rounded-2xl p-6 shadow-2xl z-30 pointer-events-none animate-[slideIn_0.5s_ease-out]">
